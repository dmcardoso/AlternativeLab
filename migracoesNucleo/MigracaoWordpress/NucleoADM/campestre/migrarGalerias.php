...
$conOrigem = new PDO("mysql:dbname=" . BD_ORIGEM . ";host=localhost", "root", "", $options = array(
...
try {
    ...
    $query = $conOrigem->prepare("
        SELECT
	      g.*,
	      (select midia from midias m where m.id = ms.midia_id ) as foto
          FROM
	      galerias g 
	      LEFT JOIN midias_uso ms on ms.id = g.capa
          WHERE
	      g.formatos = 'img' 
	      AND g.id != 4
    ");
    ...
    if ($bool) {
        ...
        foreach ($result as $i => $v) {
            ...
            echo "Galeria {$post_id} - {$titulo} : SUCESSO!" . "<br>";
            $fileInfo = null;
            $ext_file = null;
