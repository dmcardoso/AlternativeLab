...
try {
    ...
    $query = $conOrigem->prepare("
        select id, title as titulo, alias as slug, introtext as texto, created as data from m67fm_k2_items "
    );
    ...
    if ($bool) {
        ...
        foreach ($result as $i => $v) {
            ...
            $regex_not_noticia = "/^([0-9]{3})\/([0-9]{4})$/";
            if (!preg_match($regex_not_noticia, trim($titulo))) {
                $contadorNoticias++;
                ...
                // capas
                $nome_capa = 'Image' . $v['id'];
                $nome_capa = md5($nome_capa) . ".jpg";
                $arquivo_move_capa = "novo/{$relacao_pasta}{$post_id}_capa_{$nome_capa}";
                if (file_exists("antigo/{$nome_capa}")) {
                    copy("antigo/{$nome_capa}", $arquivo_move_capa);
                }

                echo "Notícia {$post_id} : SUCESSO!" . "<br>";

                $query_galerias = $conOrigem->prepare("SELECT gi.name as foto FROM m67fm_gallery_galleries gg inner join m67fm_gallery_images gi on gg.id = gi.gallery_id where gg.itemcontentid = ?");
                $query_galerias->bindValue(1, $v['id']);
                ...
                if (count($result_galerias) > 0) {
                    ...
                    foreach ($result_galerias as $idx => $val) {
                        ...
                        $arquivo_move = "novo/{$relacao_pasta}{$post_id}_galeria_{$midia_galeria}";
                        if (!file_exists($arquivo_move)) {
                            copy("antigo/{$v['id']}/{$midia_galeria}", $arquivo_move);
                        }
                        $cont++;
    ...
    echo "{$contadorNoticias} notícias migradas <br>";
