...
$conOrigem = new PDO("mysql:dbname=mpx_carmolandia;host=localhost", "root", "1234", $options = array(
    PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8',
    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
    PDO::ATTR_PERSISTENT => false
));

$conDestino = new PDO("mysql:dbname=padrao_carmolandia;host=localhost", "root", "1234", $options = array(
    PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8',
    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
    PDO::ATTR_PERSISTENT => false
));
...
try {
    ...
    $query = $conOrigem->prepare("
        SELECT
            ...
            n.titulo,
            n.foto,
            (SELECT nome from bancoimagens b where b.foto = n.foto) as titulo_capa
        FROM
            noticias n "
    );
...
//        print_r($result);
        ...
        foreach ($result as $i => $v) {
            ...
            $titulo_capa = removeAcentos($v['titulo_capa'], "-");
            ...
            if (file_exists($arquivo_move_capa)) {
                ...
                while (file_exists($arquivo_move_capa)) {
                    $arquivo_move_capa = "novo/{$relacao_pasta}{$post_id}_capa_{$titulo_capa}-({$conta}).{$ext_file}";
                    $conta++;
                }
            }
        ...
        SELECT
            n.foto,
            (SELECT nome from bancoimagens b where b.foto = n.foto) as titulo_foto_galeria
        FROM
            noticias_fotos n 
        WHERE cod_noticias = {$id}"
            );
            ...
            if (count($result_galerias) > 0) {
                ...
                foreach ($result_galerias as $idx => $val) {
                    ...
                    $titulo_foto_galeria = removeAcentos($val['titulo_foto_galeria'], "-");
                    ...
                    if (file_exists($arquivo_move)) {
                        ...
                        while (file_exists($arquivo_move)) {
                            $conta++;
                            $arquivo_move = "novo/{$relacao_pasta}{$post_id}_galeria_{$titulo_foto_galeria}-({$conta}).{$ext_file}";
                        }
                    }
